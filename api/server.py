...
# 使用ContextVar实现线程安全的依赖注入
_app_config: ContextVar[Optional[MemoryConfig]] = ContextVar("app_config", default=None)
_manager_instance: ContextVar[Optional[MemoryManager]] = ContextVar(
    "manager_instance", default=None
)
...
def get_manager() -> MemoryManager:
    """获取记忆管理器实例 (线程安全)"""
    # 先尝试获取已存在的实例
    manager = _manager_instance.get()
    if manager is not None:
        return manager

    # 创建新实例
    config = _app_config.get()
    if config is None:
        config = MemoryConfig()
    manager = MemoryManager(config)
    _manager_instance.set(manager)
    return manager


def set_app_config(config: Optional[MemoryConfig] = None) -> None:
    """设置应用配置 (启动时调用)"""
    _app_config.set(config or MemoryConfig())
...
async def start_session(
    request: StartSessionRequest, manager: MemoryManager = Depends(get_manager)
):
    """开始新会话"""
    try:
...
async def add_message(
    request: AddMessageRequest, manager: MemoryManager = Depends(get_manager)
):
    """添加消息到会话"""
    try:
        ...
        manager.add_message(
            request.session_id, request.role, request.content, request.metadata
        )
...
async def end_session(
    request: EndSessionRequest, manager: MemoryManager = Depends(get_manager)
):
    """结束会话并提取记忆"""
    try:
...
async def get_memory_context(
    request: GetContextRequest, manager: MemoryManager = Depends(get_manager)
):
    """获取记忆上下文（用于增强LLM对话）"""
    try:
        context = await manager.get_memory_context(request.session_id, request.query)
...
async def update_user_profile(
    request: UpdateProfileRequest, manager: MemoryManager = Depends(get_manager)
):
    """更新用户画像"""
...
async def cleanup_old_data(
    days: int = 7, manager: MemoryManager = Depends(get_manager)
):
    """清理过期数据"""
...
async def export_user_memory(
    user_id: str, manager: MemoryManager = Depends(get_manager)
):
    """导出用户记忆"""
...
async def import_user_memory(
    data: Dict[str, Any], manager: MemoryManager = Depends(get_manager)
):
    """导入用户记忆"""
...
def create_app(config: Optional[MemoryConfig] = None) -> FastAPI:
    """创建应用实例"""
